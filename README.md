...
1. **选模式**：顶部紧凑的浅色／深色／跟随系统按钮，下方选择柔和／清晰及头部、角色、物件、设置角色四项装饰。外观设置按家族独立保存。
2. **选主题**：搜索中英文名称，使用全部／动漫／游戏／其他分类筛选。卡片左右展示浅深色板，并用现有角色小图辅助辨认；简约版与原主题相邻，分类和搜索保留配对。分类维护于 `families/categories.json`。
3. **滚动主题库**：滚动仅限卡片区域，上方设置、搜索分类与底部应用按钮保持可见。没有场景预览。
4. **应用或取消**：点击「应用」才保存。取消修改、切换设置页或关闭窗口会丢弃未应用草稿。「恢复默认」只重置当前家族的外观草稿，应用后生效。

选择「默认」并应用会交还内建外观，保留其他家族的个性化设置。清晰模式沿用原有宽屏规则；横幅在 ≤900px 隐藏，左下角物件在 ≤1050px 隐藏。

偏好按浏览器保存在 `localStorage["dsh-themes:pref"]`，使用版本 2 的 `{ version, selected, families }` 结构。旧 `{family, mode, style, wallpaper}` 自动迁移，旧清晰度仅归入原选中家族；不提供跨设备同步。
