...
当前收录家族（28 个、56 套浅深色皮肤，按呈现风格分两类）：
...
1. **选模式**：顶部紧凑的浅色／深色／跟随系统按钮，下方选择清晰／柔和（默认清晰：角色与物件更鲜明；柔和：装饰淡化，减少阅读干扰）及头部、角色、物件、设置角色四项装饰。明暗模式、清晰度和装饰选项按家族独立自动保存。
2. **选主题**：搜索中英文名称，使用全部／动漫／游戏／其他分类筛选。选中即预览，并恢复该主题记住的配置。卡片左右展示浅深色板，并用现有角色小图辅助辨认；简约版与原主题相邻，分类和搜索保留配对。分类维护于 `families/categories.json`。
...
偏好按浏览器保存在 `localStorage["dsh-themes:pref"]`，使用版本 3 的 `{ version, selected, families }` 结构。旧 `{family, mode, style, wallpaper}` 和版本 2 自动迁移，保留原选中家族的明暗模式与各家族装饰配置，其他家族未记录的明暗模式默认跟随系统；不提供跨设备同步。
