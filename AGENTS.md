...
Harness runtime homes must stay outside OneDrive and all other synced directories. Never set `DSH_HOME` to the scratch dir or create `*-home`, profile dependencies, `node_modules`, junctions, or symlinks inside it. On CASTLE, use `D:/Documents/dsh-theme-runtime/<purpose>-home` for isolated checks; on other machines choose a local, non-synced directory. Set `DSH_HOME` only for the relevant process/session so normal Harness profiles remain unchanged. Only source images, scripts, logs, and screenshots belong in the synced scratch dir. Renaming that dir does not prevent Harness from creating dependency junctions under a configured `DSH_HOME`.
